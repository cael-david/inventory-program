...
async function loadProductAndBatches() {
    ...
    // Soma do "current quantity" (quantity_original é o current na exibição corrigida)
    let total = 0;
    batches.forEach(l => total += l.quantity_original);
    ...
    if (batches.length === 0) {
        ...
        listaLotes.innerHTML = batches.map(l => `
            <li>
                ...
                <strong>Original quantity:</strong> ${l.quantity_total} | 
                <strong>Current quantity:</strong> ${l.quantity_original} | 
